...
def run_backtest(config_path: str, start_date: str, end_date: str, 
    ...
    market_idx = config.market_proxies.market_index_symbol
    vol_idx = config.market_proxies.volatility_symbol
    risk_idx = config.market_proxies.risk_asset_symbol
    
    if daily_fast_track:
        # Get all trading days for daily monitoring (Fast Risk-Off + Stop-Loss)
        daily_dates = preprocessor.daily_data[market_idx].index
    ...
    for i, date in enumerate(decision_dates, 1):
        try:
            ...
            daily_prices = preprocessor.get_daily_data_as_of(date, symbols=[market_idx, vol_idx, risk_idx])
            ...
            # Priority 2: Fast Risk-Off (only if not weekly)
                ...
                should_trigger_fast, signals = check_fast_risk_off_trigger(
                    daily_prices.get(market_idx),
                    daily_prices.get(risk_idx),
                    daily_prices.get(vol_idx),
                    date,
                    config
                )
