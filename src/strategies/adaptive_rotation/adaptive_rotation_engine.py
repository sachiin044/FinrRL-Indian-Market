...
class AdaptiveRotationEngine:
    ...
    def _detect_regime(
        ...
        market_idx = self.config.market_proxies.market_index_symbol
        vol_idx = self.config.market_proxies.volatility_symbol
        risk_idx = self.config.market_proxies.risk_asset_symbol

        spx_weekly = prices.get(market_idx, pd.Series())
        vix_weekly = prices.get(vol_idx, pd.Series())
        qqq_weekly = prices.get(risk_idx, pd.Series())
        ...
        if self.data_preprocessor is not None:
            try:
                daily_data = self.data_preprocessor.get_daily_data_as_of(
                    as_of_date=as_of_date,
                    symbols=[market_idx, vol_idx, risk_idx]
                )
                spx_daily = daily_data.get(market_idx, spx_weekly)
                vix_daily = daily_data.get(vol_idx, vix_weekly)
                qqq_daily = daily_data.get(risk_idx, qqq_weekly)
    ...
    def _detect_exceptions(
        ...
        benchmark_symbol = self.config.benchmark.excess_return_benchmark
