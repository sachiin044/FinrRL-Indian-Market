...
class MarketProxyConfig(BaseModel):
    """Market proxy symbols for regime detection."""
    market_index_symbol: str = "^GSPC"   # Broad market index (price shock + trend)
    risk_asset_symbol: str = "QQQ"       # Risk-on proxy (price shock)
    volatility_symbol: str = "^VIX"      # Volatility proxy (vol shock)
...
class AdaptiveRotationConfig(BaseModel):
    ...
    market_proxies: MarketProxyConfig = Field(default_factory=MarketProxyConfig)
    ...
    def get_required_symbols(self) -> List[str]:
        ...
        - Benchmark symbol
        - Market proxy symbols (market index, volatility, risk asset)
        ...
        # Add market proxies
        for index in [
            self.market_proxies.market_index_symbol,
            self.market_proxies.volatility_symbol,
            self.market_proxies.risk_asset_symbol
        ]:
            if index and index not in required:
