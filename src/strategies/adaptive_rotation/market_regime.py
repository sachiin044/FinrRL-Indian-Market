...
def detect_price_shock(
    ...
    # Ensure DatetimeIndex for robust comparisons
...
def detect_volatility_shock(
    ...
    """
    if vix_prices is None or len(vix_prices) == 0:
        return False, 0.0, 0.0

    # Ensure DatetimeIndex for robust comparisons
    if not isinstance(vix_prices.index, pd.DatetimeIndex):
        vix_prices = vix_prices.copy()
        vix_prices.index = pd.to_datetime(vix_prices.index, errors="coerce")
        vix_prices = vix_prices[~vix_prices.index.isna()]
    # Get data as of date
